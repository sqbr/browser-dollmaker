...
const none = {name:"None",location: "", colour: false, hasFullRows: false, topcorner:[0,0]}
...
for (let i =0; i<sprite_body_list.length;i++){
    ...
    sprite_torso_list.push({name: b, location: "body/"+b, colour: true, hasFullRows: false,topcorner:[0,0]})
}
...
for (let i =0; i<height_list.length;i++){
    b= height_list[i];
    sprite_shoes_list.push({name: b, location: "shoes/"+b, colour: true, hasFullRows: false,topcorner:[0,0]})
}

const sprite_pants_list = []
sprite_pants_list.push({name: "Shorts", location: "pants/pants", colour: true, hasFullRows: false,topcorner:[0,0]})

const sprite_pants_names = sprite_pants_list.map(nameOf);

const sprite_shirt_list = []
sprite_shirt_list.push({name: "Overalls", location: "shirts/shirts", colour: false, hasFullRows: true,topcorner:[0,0]})

const sprite_accessory_list = []
sprite_accessory_list.push({name: "Glasses", location: "accessory/accessories", colour: false, hasFullRows: false,topcorner:[0,32]})

const sprite_accessory_names = sprite_accessory_list.map(nameOf);

const sprite_facialhair_list = []
sprite_facialhair_list.push({name: "Beard", location: "facialhair/facialhair", colour: true, hasFullRows: false,topcorner:[0,0]})
...
for (let row =0; row<7;row++)
    for (let column =0; column<8;column++){
        if (row*8+column< hairstyles_names.length)
            sprite_hair_list.push({name: hairstyles_names[row*8+column], location: "hair/hairstyles", colour: true, hasFullRows: false,topcorner:[16*column,96*row]})
}
for (let row =0; row<3;row++)
    for (let column =0; column<8;column++){
        if (row*8+column<hairstyles2_names.length)
            sprite_hair_list.push({name: hairstyles2_names[row*8+column], location: "hair/hairstyles2", colour: true, hasFullRows: false,topcorner:[16*column,128*row]})
}
const sprite_hair_names = sprite_hair_list.map(nameOf);

const sprite_hat_list = []
sprite_hat_list.push({name: "Santa hat", location: "hats/hats", colour: false, hasFullRows: true,topcorner:[23,159]})

const sprite_arms_list = []
for (let i =0; i<sprite_body_list.length;i++){
    b= sprite_body_list[i];
    sprite_arms_list.push({name: b, location: "body/"+b, colour: true, hasFullRows: false,topcorner:[96,0]})
}
